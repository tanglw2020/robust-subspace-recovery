...
int main(int argc, const char **argv) {
  ...
  for (int i = 0; i < num_seg; ++i) {
      ...
      repeat_run_PSGD(N+ n_step*i, r, D, d, sigma_limit, 400, cos_phi_PSGD[i]);
  ...
  for (int i = 0; i < num_seg; ++i) {
      float rt = ratios(i);
      repeat_run_FMS(N + n_step*i, r, D, d, sigma_limit, 400, cos_phi_REAPER[i]);
  ...
  for (int i = 0; i < num_seg; ++i) {
      ...
      repeat_run_IRLS_PSGD(N+ n_step*i, r, D, d, sigma_limit, 400, cos_phi_PSGD_IRLS[i]);
